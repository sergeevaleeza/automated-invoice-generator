...
- After seeing the Phase 0 output actually rendered, two of the fixture-derived decisions above turned out wrong in practice: column widths were revised to explicit on-screen pixel targets (A=105px, B=64px, C=225px, D=140px, E=140px — converted via Excel's standard `width = (pixels − 5) / 7` formula, which reproduces Excel's canonical 8.43-unit default for the 64px target and validates the conversion), giving the payment-notice box's D:E span much more room (E was previously left at the ~64px implicit default, which was too narrow for the notice text to wrap the way it was authored). All five columns are now set explicitly (B and E are no longer left at Excel's implicit default).
