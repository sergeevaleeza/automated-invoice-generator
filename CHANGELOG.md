...
#### 2. Regression guard: `tests/test_envelope_generation.py` (new, 8 tests)
- Page size/orientation/`w:code="20"`/margins match the #10 spec exactly; clinic return address and patient delivery address (from `PatientData`) are present; the company-name line is asserted to appear **exactly once** (directly guards against the near-miss above); no letter-body or placeholder-scaffold text (`Dear `, `Sincerely`, `Please find enclosed`, `Patient Record Number`, `PLACEHOLDER TEMPLATE`) appears anywhere in the generated envelope; and a standalone test greps all bundled `.docx` templates for the placeholder scaffold text so it can't silently ship again. Uses a raw-XML text extraction helper (not `Document.paragraphs`) specifically so content-control-wrapped text like the company name is actually checked. Full suite (109/109) unaffected otherwise.

#### 3. Open question — not acted on, needs your confirmation
- `invoice_app.py` validates the active cover-letter template against `REQUIRED_TEMPLATE_PLACEHOLDERS` (`invoice_models.py`), which still expects `[Full Name]` and `[Patient Record Number]` — placeholders a pure envelope has no reason to include (it uses `[First Name] [Last Name]` split, and no PRN at all). This means the Settings tab now shows a "missing placeholders" warning for the bundled default template, even though the template is correct for what it actually produces today. Left as-is pending the cover-letter question below, since the right fix (redefine the placeholder contract for an envelope vs. reintroducing a separate cover-letter output with its own placeholder set) depends on the answer.
- Per the task instructions: **not building a separate cover-letter output** (wired to `templates/Access_Multi_Letter_Cover.docx` via config) without confirmation that it's actually meant to be a distinct deliverable from the envelope — asked in this session, not assumed.
